...
app.patch("/api/accounts/:username", (req, res) => {
  const accounts = store.getAccounts();
  const info = accounts[req.params.username];
  if (!info) return res.status(404).json({ error: "계정을 찾을 수 없습니다." });

  if (typeof req.body.pushEnabled === "boolean") {
    info.pushEnabled = req.body.pushEnabled;
  }
